...
FOR EACH ROW
BEGIN
    DECLARE end_time_slot INT;
    SET end_time_slot = NEW.time_slot + 1; -- Calculate end time slot based on match duration
    ...
    IF EXISTS (
        ...
        WHERE (NEW.stadium_ID = stadium_ID)
          AND (NEW.date = date)
          AND ((NEW.time_slot BETWEEN time_slot AND time_slot + 1)
              OR (end_time_slot BETWEEN time_slot AND time_slot + 1)
              OR (NEW.time_slot <= time_slot AND end_time_slot >= time_slot + 1))
    ) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Cannot insert match session. Overlaps with existing session.';
    END IF;
